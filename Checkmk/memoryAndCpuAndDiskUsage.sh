...
echo "checkmk,timestamp,cpu_usage(in %),memory_usage,disk_usage(in KB)" > cpu_mem_disk_usageDataCheckmk.csv
...
while (("$COUNTER" < "$MAX_ITERATIONS")); do
    ...
    USAGE=$(du -sk ./checkmk_data/cmk/var/pnp4nagios/perfdata | awk '{print $1}' | sed 's/K//');
    docker stats --no-stream --format "{{.Name}},{{.CPUPerc}},{{.MemUsage}}" "$CONTAINER_NAME" | while IFS=',' read -r NAME CPU MEM; do
        ...
        echo "$CONTAINER_NAME,$TIMESTAMP,$CPU,$MEM_USED,$USAGE" >> cpu_mem_disk_usageDataCheckmk.csv;
    done
    sleep 15;
done
